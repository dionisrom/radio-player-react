...
function StationCard({ station, onPlay, isFav, onToggleFav, nowPlaying = '' }) {
  ...
  return (
  <div className="flex flex-col h-full min-h-[220px] p-4 rounded-lg shadow-lg bg-white/40 dark:bg-gray-800/40 backdrop-blur-md border-[1px] border-gray-300 dark:border-gray-700 hover:shadow-2xl transition-shadow duration-200">
      <div className="shrink-0 mb-3">
        <div className="w-full h-28 bg-white/10 dark:bg-black/10 rounded-md flex items-center justify-center overflow-hidden" style={{ minHeight: '5.5rem' }}>
          <img
            src={station.favicon ? station.favicon : defaultIcon}
            alt={station.name}
